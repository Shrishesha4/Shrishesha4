...
export function optimizeImage(url: string, options: Partial<ImageOptimizationOptions> = {}) {
  if (!url) return "";
  ...
  const cloudName = process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || "optimage";
